"""
circuit_1hop.py — Experiment 1: 1-hop circuit load test.

Each Locust user builds its own circuit per request:
  client → guard → destination (guard acts as exit)
...
Environment variables:
  ...
  S3_PREFIX                  S3 key prefix (default: experiment-1/1-hop)

Usage:
  locust -f circuit_1hop.py --headless --csv=results/1hop \\
         --host $DIRECTORY_URL
"""
...
S3_PREFIX = os.getenv("S3_PREFIX", "experiment-1/1-hop").rstrip("/")
...
class CircuitOneHopUser(HttpUser):
    """
    Each user builds a fresh 1-hop circuit per task.
    Uses a plain requests.Session so it can talk to both the directory
    server and the guard node (which may differ from self.client's host).
    """
    wait_time = between(0.01, 0.05)
    # host is set to DIRECTORY_URL so Locust can reach the directory;
    # actual circuit traffic goes via the guard node directly.
...
def upload_results_to_s3(environment, **kwargs):
    if not S3_BUCKET:
        log.info("[1hop] EXPERIMENT_RESULTS_BUCKET not set — skipping S3 upload")
        return
    ...
    if not rows:
        log.warning("[1hop] no stats to upload")
        return
    ...
    try:
        ...
        log.info("[1hop] uploaded results to s3://%s/%s", S3_BUCKET, key)
    except (BotoCoreError, ClientError) as exc:
        log.error("[1hop] S3 upload failed: %s", exc)
